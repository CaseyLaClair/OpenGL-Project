...
GLfloat lastMouseX = 400, lastMouseY = 300; //locks mouse cursor at center of the screen
...
bool mouseDetected = false; //initially false
...
void URenderGraphics(void){
	...
	glm::mat4 model;
	model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f)); //place the object at the center of viewport
	model = glm::rotate(model, 45.0f, glm::vec3(0.0f, 1.0f, 0.0f)); //rotate the object 45 degrees on the x axis
	model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f)); //Increase the object size
